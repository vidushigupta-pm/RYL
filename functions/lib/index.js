...
exports.quarterlyProductReview = exports.chatAboutProduct = exports.searchProductByName = exports.analyseLabel = void 0;
...
function withTimeout(promise, ms = 300000) {
    return Promise.race([
        promise,
...
function checkRateLimit(uid) {
    const now = Date.now();
    const calls = (userCallLog.get(uid) || []).filter(t => now - t < 60000);
    if (calls.length >= RATE_LIMIT_PER_MIN) {
        throw new https_1.HttpsError('resource-exhausted', 'Too many requests. Please wait a moment and try again.');
    }
    userCallLog.set(uid, [...calls, now]);
}
async function getIngredientDetails(ai, unknownIngredients, category) {
...
exports.analyseLabel = (0, https_1.onCall)({ secrets: [GEMINI_API_KEY] }, async (request) => {
    const { backImageBase64, backMimeType, frontImageBase64, frontMimeType } = request.data;
    if (!backImageBase64 || !backMimeType) {
        throw new https_1.HttpsError("invalid-argument", "Back image data and mime type are required.");
    }
    try {
  ...
  "suggestions": [{ "type": "GENERIC", "name": "...", "reason": "..." }]
    ...
    Return ONLY JSON matching the AnalysisResult structure.`;
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", `Failed to analyze label: ${error.message || 'Unknown error'}`);
    }
});
exports.searchProductByName = (0, https_1.onCall)({ secrets: [GEMINI_API_KEY] }, async (request) => {
    ...
    try {
        const apiKey = GEMINI_API_KEY.value();
        const ai = new genai_1.GoogleGenAI({ apiKey });
        const ragCacheResult = await (0, ragService_1.ragLookup)({ productName });
    ...
    Return ONLY JSON matching the AnalysisResult structure.`;
        ...
        throw new https_1.HttpsError("internal", `Failed to search product "${productName}": ${errorDetails}`);
    }
});
exports.chatAboutProduct = (0, https_1.onCall)({ secrets: [GEMINI_API_KEY] }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
    const { productAnalysis, userMessage, profile, history } = request.data;
    if (!productAnalysis || !userMessage || !profile) {
        throw new https_1.HttpsError("invalid-argument", "Product analysis, user message, and profile are required.");
    }
    try {
        const apiKey = GEMINI_API_KEY.value();
        const ai = new genai_1.GoogleGenAI({ apiKey });
...
USER QUESTION: ${userMessage}`;
        ...
        throw new https_1.HttpsError("internal", "Failed to chat about product.");
    }
});
exports.quarterlyProductReview = (0, scheduler_1.onSchedule)({ schedule: '0 0 1 1,4,7,10 *', timeZone: 'Asia/Kolkata' }, async () => {
