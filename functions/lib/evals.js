...
function getKey(entry) {
    return Object.entries(data_1.INGREDIENT_DB).find(([, v]) => v === entry)?.[0];
}
function assertLookupKey(rawName, expectedKey, desc) {
    const result = (0, data_1.lookupIngredient)(rawName);
    const label = desc || `lookup("${rawName}")`;
    if (expectedKey === null) {
        if (result === null)
            ok(`${label} → correctly null`);
        else
            fail(`${label} → expected null but matched "${getKey(result)}"`);
    }
    else {
        if (result === null)
            fail(`${label} → expected "${expectedKey}" but got null`);
        else {
            const foundKey = getKey(result);
            if (foundKey === expectedKey)
                ok(`${label} → "${expectedKey}"`);
            else
                fail(`${label} → expected "${expectedKey}" but matched "${foundKey}" (${result.common_names[0]})`);
        }
    }
}
function assertScoreRange(label, score, min, max) {
    if (score >= min && score <= max)
        ok(`${label} → score ${score} (expected ${min}–${max})`);
    else
        fail(`${label} → score ${score} is OUTSIDE expected range ${min}–${max}`);
}
const FULL_COVERAGE = { verified: [], unverified: [], coveragePercent: 100 };
...
let totalAliases = 0;
const allAliasesSeen = new Map();
for (const [key, entry] of Object.entries(data_1.INGREDIENT_DB)) {
    if (!entry.common_names || entry.common_names.length === 0) {
        fail(`[${key}] empty common_names`);
        integrityFails++;
    }
    if (!entry.function) {
        fail(`[${key}] missing "function"`);
        integrityFails++;
    }
    if (!VALID_TIERS.includes(entry.safety_tier)) {
        fail(`[${key}] invalid safety_tier "${entry.safety_tier}"`);
        integrityFails++;
    }
    if (!VALID_FSSAI.includes(entry.fssai_status)) {
        fail(`[${key}] invalid fssai_status "${entry.fssai_status}"`);
        integrityFails++;
    }
    if (!VALID_QUALITY.includes(entry.data_quality)) {
        fail(`[${key}] invalid data_quality "${entry.data_quality}"`);
        integrityFails++;
    }
    if (typeof entry.score_impact !== 'number') {
        fail(`[${key}] score_impact not a number`);
        integrityFails++;
    }
    if (!entry.plain_explanation || entry.plain_explanation.length < 20) {
        fail(`[${key}] plain_explanation missing/too short`);
        integrityFails++;
    }
    ...
    for (const alias of entry.common_names) {
        totalAliases++;
        const normAlias = (0, data_1.normaliseIngredientName)(alias);
    ...
    for (const flag of entry.condition_flags || []) {
        if (!flag.condition) {
            fail(`[${key}] condition_flag missing "condition"`);
            integrityFails++;
        }
        if (!flag.source)
            warn(`[${key}] condition_flag "${flag.condition}" has no source citation`);
    }
}
console.log(`  DB has ${Object.keys(data_1.INGREDIENT_DB).length} entries, ${totalAliases} total aliases`);
if (integrityFails === 0)
    ok(`All DB entries pass integrity checks`);
...
for (const [key, entry] of Object.entries(data_1.INGREDIENT_DB)) {
    for (const alias of entry.common_names) {
        ...
        if (normAlias.length < 4)
            continue;
        const found = (0, data_1.lookupIngredient)(alias);
        if (!found) {
...
if (selfFails === 0)
    ok(`All aliases resolve back to their own entry`);
...
for (const [input, badKey, desc] of FP_CASES) {
    const result = (0, data_1.lookupIngredient)(input);
    if (result === null) {
...
for (const [input, expectedKey, desc] of LOOKUP_CASES) {
    ...
    if (expectedKey === null) {
        if (result === null)
            ...
            warn(`"${input}" → matched "${getKey(result)}" — expected null — ${desc} — verify if correct`);
        }
    }
    else {
        if (result === null) {
            fail(`"${input}" → null, expected "${expectedKey}" — ${desc}`);
            lookupFails++;
        }
        else {
            const foundKey = getKey(result);
            if (foundKey === expectedKey)
                ok(`"${input}" → "${expectedKey}" (${desc})`);
            else {
                fail(`"${input}" → "${foundKey}", expected "${expectedKey}" — ${desc}`);
                lookupFails++;
            }
        }
    }
...
for (const [ingredients, expectedPct, desc] of BATCH_CASES) {
    const result = (0, data_1.batchLookupIngredients)(ingredients);
    if (result.coveragePercent === expectedPct)
        ok(`${desc} → ${result.coveragePercent}%`);
    else
        fail(`${desc} → expected ${expectedPct}% but got ${result.coveragePercent}%`);
}
section('SUITE 6: calculateScore() — Nutrition Tiers & Logic');
function scoreBreakdown(score, label) {
    return score.score_breakdown.find(b => b.label.includes(label));
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 50 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sugar');
    if (b?.impact === -10)
        ok(`Very High Sugar (50g) → impact -10`);
    else
        fail(`Very High Sugar (50g) → expected -10, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 25 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sugar');
    if (b?.impact === -6)
        ok(`High Sugar (25g) → impact -6`);
    else
        fail(`High Sugar (25g) → expected -6, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 15 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sugar');
    if (b?.impact === -3)
        ok(`Moderate Sugar (15g) → impact -3`);
    else
        fail(`Moderate Sugar (15g) → expected -3, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 5 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sugar');
    if (!b)
        ok(`Low Sugar (5g) → no sugar deduction`);
    else
        fail(`Low Sugar (5g) → unexpected deduction: ${b.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sodium_mg: 1200 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sodium');
    if (b?.impact === -18)
        ok(`Extremely High Sodium (1200mg) → impact -18`);
    else
        fail(`Extremely High Sodium → expected -18, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sodium_mg: 900 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sodium');
    if (b?.impact === -13)
        ok(`Very High Sodium (900mg) → impact -13`);
    else
        fail(`Very High Sodium → expected -13, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sodium_mg: 600 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sodium');
    if (b?.impact === -7)
        ok(`High Sodium (600mg) → impact -7`);
    else
        fail(`High Sodium → expected -7, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sodium_mg: 200 }, 'FOOD');
    const b = scoreBreakdown(s, 'Sodium');
    if (!b)
        ok(`Low Sodium (200mg) → no deduction`);
    else
        fail(`Low Sodium (200mg) → unexpected deduction: ${b.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { trans_fat_g: 0.8 }, 'FOOD');
    const b = scoreBreakdown(s, 'Trans Fat');
    if (b?.impact === -10)
        ok(`Trans Fat >0.5g → impact -10`);
    else
        fail(`Trans Fat >0.5g → expected -10, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { trans_fat_g: 0.3 }, 'FOOD');
    const b = scoreBreakdown(s, 'Trans Fat');
    if (b?.impact === -4)
        ok(`Trace Trans Fat (0.3g) → impact -4`);
    else
        fail(`Trace Trans Fat → expected -4, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { protein_g: 25 }, 'FOOD');
    const b = scoreBreakdown(s, 'Protein');
    if (b?.impact === 8)
        ok(`High Protein (25g) → +8`);
    else
        fail(`High Protein → expected +8, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { fibre_g: 8 }, 'FOOD');
    const b = scoreBreakdown(s, 'Fibre');
    if (b?.impact === 6)
        ok(`High Fibre (8g) → +6`);
    else
        fail(`High Fibre → expected +6, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { fibre_g: 4 }, 'FOOD');
    const b = scoreBreakdown(s, 'Fibre');
    if (b?.impact === 3)
        ok(`Good Fibre (4g) → +3`);
    else
        fail(`Good Fibre → expected +3, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, {}, 'FOOD', true);
    const b = scoreBreakdown(s, 'Ultra-Processed');
    if (b?.impact === -10)
        ok(`UPF (FOOD) → -10 penalty`);
    else
        fail(`UPF FOOD → expected -10, got ${b?.impact}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, {}, 'COSMETIC', true);
    const b = scoreBreakdown(s, 'Ultra-Processed');
    if (!b)
        ok(`UPF on COSMETIC → no penalty (correct)`);
    else
        fail(`UPF COSMETIC → should not apply, got ${b?.impact}`);
}
{
    const low = { verified: [], unverified: ['a', 'b', 'c', 'd', 'e'], coveragePercent: 0 };
    const s = (0, scoringEngine_1.calculateScore)(low, null, 'FOOD');
    if (s.is_suppressed)
        ok(`0% coverage → score suppressed`);
    else
        fail(`0% coverage → NOT suppressed, got score ${s.overall_score}`);
}
{
    const terrible = (0, data_1.batchLookupIngredients)(['Tartrazine', 'Sunset Yellow', 'Sodium Benzoate', 'TBHQ', 'Aspartame']);
    const s = (0, scoringEngine_1.calculateScore)(terrible, { sugar_g: 60, sodium_mg: 1500, trans_fat_g: 2 }, 'FOOD', true);
    if (s.overall_score >= 0 && s.overall_score <= 100)
        ok(`Score clamped: ${s.overall_score}`);
    else
        fail(`Score out of bounds: ${s.overall_score}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 60, sodium_mg: 1500 }, 'COSMETIC');
    const sugarBlock = scoreBreakdown(s, 'Sugar');
    const sodiumBlock = scoreBreakdown(s, 'Sodium');
    if (!sugarBlock && !sodiumBlock)
        ok(`Nutrition scoring correctly skipped for COSMETIC`);
    else
        fail(`COSMETIC incorrectly applied nutrition scoring`);
}
section('SUITE 7: Known Product Score Profiles (real-world sanity check)');
{
    const ingredients = (0, data_1.batchLookupIngredients)(['Monosodium Glutamate', 'TBHQ', 'Sodium Triphosphate', 'Tartrazine']);
    const s = (0, scoringEngine_1.calculateScore)(ingredients, { sodium_mg: 890, sugar_g: 3, protein_g: 8 }, 'FOOD', true);
...
{
    const ingredients = (0, data_1.batchLookupIngredients)(['INS 500', 'INS 471']);
    const s = (0, scoringEngine_1.calculateScore)(ingredients, { sugar_g: 22, sodium_mg: 180, protein_g: 7, fibre_g: 1 }, 'FOOD', false);
    assertScoreRange('Parle-G Biscuits (approx)', s.overall_score, 75, 95);
    console.log(`    Breakdown: ${s.score_breakdown.map(b => `${b.label}(${b.impact > 0 ? '+' : ''}${b.impact})`).join(', ')}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 57, sodium_mg: 40, protein_g: 6, fibre_g: 3 }, 'FOOD', true);
    assertScoreRange('Nutella (approx)', s.overall_score, 62, 82);
    console.log(`    Breakdown: ${s.score_breakdown.map(b => `${b.label}(${b.impact > 0 ? '+' : ''}${b.impact})`).join(', ')}`);
}
{
    const s = (0, scoringEngine_1.calculateScore)(FULL_COVERAGE, { sugar_g: 3, sodium_mg: 150, protein_g: 30, fibre_g: 2 }, 'SUPPLEMENT', false);
    assertScoreRange('Protein Supplement (clean)', s.overall_score, 100, 100);
    console.log(`    Score: ${s.overall_score}`);
}
{
    const ingredients = (0, data_1.batchLookupIngredients)(['Methylparaben', 'Sodium Lauryl Sulfate']);
    const s = (0, scoringEngine_1.calculateScore)(ingredients, null, 'COSMETIC', false);
    assertScoreRange('Face Wash (parabens + SLS)', s.overall_score, 90, 100);
    console.log(`    Breakdown: ${s.score_breakdown.map(b => `${b.label}(${b.impact > 0 ? '+' : ''}${b.impact})`).join(', ')}`);
}
{
    const ingredients = (0, data_1.batchLookupIngredients)(['Mercury', 'Lead Acetate']);
    const s = (0, scoringEngine_1.calculateScore)(ingredients, null, 'COSMETIC', false);
...
for (const [input, expected] of NORM_CASES) {
    const actual = (0, data_1.normaliseIngredientName)(input).replace(/\s+/g, ' ').trim();
    const exp = expected.replace(/\s+/g, ' ').trim();
    if (actual === exp)
        ok(`normalise("${input}") → "${actual}"`);
    else
        warn(`normalise("${input}") → "${actual}" (expected "${exp}")`);
}
console.log('\n' + '═'.repeat(60));
